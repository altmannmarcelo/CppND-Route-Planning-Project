...
bool isPositionValid(int pos) {
  ...
  cout << "Position " << pos << " is not in range between 0 and 100" << endl;
  return false;
}
int main(int argc, const char **argv) {
  ...
  // TODO 1 OK: Declare floats `start_x`, `start_y`, `end_x`, and `end_y` and
  // get user input for these values using std::cin. Pass the user input to the
  // RoutePlanner object below in place of 10, 10, 90, 90.
  float start_x, start_y, end_x, end_y;
  cout << "Enter positions between 0 and 100" << endl;
  do {
    cout << "Enter Start X position: ";
    cin >> start_x;
  } while (!isPositionValid(start_x));

  do {
    cout << "Enter Start Y position: ";
    cin >> start_y;
  } while (!isPositionValid(start_y));

  do {
    cout << "Enter End X position: ";
    cin >> end_x;
  } while (!isPositionValid(end_x));

  do {
    cout << "Enter End Y position: ";
    cin >> end_y;
  } while (!isPositionValid(end_y));
